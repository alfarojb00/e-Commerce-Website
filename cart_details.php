	...
	if(isset($_SESSION['user'])){
		...
		try{
			...
			foreach($stmt as $row){
				...
				$subtotal = ($user['user_type']=="Distributor"?$row['price'] - ($row['price'] * .25 ):$row['price']) *  $row['quantity'];
				$total += $subtotal;
				$output .= "
					<tr>
						...
						<td>".$row['name']."</td>
						<td>&#8369; ".number_format($user['user_type']=="Distributor"?$row['price'] - ($row['price'] * .25 ):$row['price'], 2)."</td>
